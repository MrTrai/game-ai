...
class GameWorld():
    ...
    def worldCollisionTest(self):
        ...
        for m1 in self.movers:
            if m1 in self.movers:
                ...
                if m1.position[0] < 0 or m1.position[0] > self.dimensions[0] or m1.position[1] < 0 or m1.position[1] > \
                        self.dimensions[1]:
                ...
                for o in self.obstacles:
                    ...
                    if needCheckVertex:
                        for v in o.getPoints():
                            ...
                            if between(v[0], m1.position[0], nextPosition[0]) and between(v[1], m1.position[1],
                                                                                          nextPosition[1]):
                    ...
                    if not needCheckVertex:
                        for l in o.getLines():
                            for r in ((m1.rect.topleft, m1.rect.topright), (m1.rect.topright, m1.rect.bottomright),
                                      (m1.rect.bottomright, m1.rect.bottomleft), (m1.rect.bottomleft, m1.rect.topleft)):
    ...
    def deleteNPC(self, npc):
        if not npc.getTeam() in self.deleted_npc:
            self.deleted_npc[npc.getTeam()] = 0
        self.deleted_npc[npc.getTeam()] += 1
        print self.deleted_npc
        if npc in self.npcs:
